...
let poin = 500
let poin_lose = -100
let Iqbal = async (m, { conn, usedPrefix, command }) => {
...
`.trim()
  ...
  conn.suit[id] = {
    chat: await conn.send2Button(m.chat, caption, footer, 'Terima', `${usedPrefix + command}terima`, 'Tolak', 'tolak', m, { contextInfo: { mentionedJid: conn.parseMention(caption) } }),
    id: id,
    p: m.sender,
    p2: m.mentionedJid[0],
    status: 'wait',
...
Iqbal.tags = ['game']
Iqbal.help = ['suitpvp', 'suit'].map(v => v + ' @tag')
Iqbal.command = /^suit(pvp)?$/i
Iqbal.limit = false
Iqbal.group = true
