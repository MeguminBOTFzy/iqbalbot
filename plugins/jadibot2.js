...
let Iqbal = async (m, { conn, args, usedPrefix, command }) => {
  let parent = args[0] && args[0] == 'plz' ? conn : global.conn
  let auth = false
if (conn.user) return
  if ((args[0] && args[0] == 'plz') || global.iqbl.user.jid == conn.user.jid) {
  ...
  } else conn.reply('Tidak bisa membuat bot didalam bot!\n\nhttps://wa.me/' + global.conm.user.jid.split`@`[0] + '?text=.jadibot')
}
Iqbal.command = /^(jadibot2|buatbot)$/i
